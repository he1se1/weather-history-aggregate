...
def get_historical_weather(lat, lon, target_month, target_day, years, include_snow=False):
    ...
    try:
        ...
        if include_snow and 'snowfall_sum' in target_days.columns:
            ...
            results.append(f"平均降雪量：{snow.mean():.1f}cm")
...
with st.sidebar:
    ...
    include_snow = st.checkbox("雪の統計を含める", value=True)
...
target_date = st.date_input("大会開催日（月日のみ使用）", value=datetime(2025, 2, 28))
...
if st.button("統計を取得してブリテン用テキストを生成", type="primary"):
    with st.spinner("データを取得中..."):
        ...
        if error:
            ...
            st.success("取得完了！")
            st.subheader("結果 (そのままコピーして要項へ)")
            st.text_area("Bulletin format", value=res_text, height=200)
            st.info("上のテキストボックスからコピーして、Wordやプログラム等の要項作成にご利用ください。")
