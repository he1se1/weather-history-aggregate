...
def get_historical_weather(lat, lon, target_month, target_day, years, include_snow=False):
    ...
    try:
        ...
        results = []
        results.append(f"過去{years}年間の{target_month}月{target_day}日の統計")
        # results.append(f"地点: 緯度 {lat}, 経度 {lon}") # 内部確認用
        ...
        if include_snow and 'snowfall_sum' in target_days.columns:
            ...
            results.append(f"1cm以上の降雪があった日数：{(snow >= 1.0).sum()}/{len(snow)}日")
